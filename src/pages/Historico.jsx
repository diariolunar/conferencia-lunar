          ...
          {carregando ? (
            ...
            <div className="history-tree">
              {Object.values(agrupado).map((sub) => {
                ...
                return (
                  <details className="history-sub-block" key={sub.subNome} open>
                    <summary>
                      <div>
                        <span>Sub</span>
                        ...
                        <small>
                          {stats.membros} membro(s) · {stats.total} conferência(s)
                        </small>
                      </div>
                    </summary>
                    ...
                    <div className="sub-actions">
                      <button
                        className="mini-button danger"
                        type="button"
                        onClick={() => limparSub(sub)}
                        disabled={limpando}
                      >
                        {limpando
                          ? "Limpando..."
                          : "Limpar histórico do sub"}
                      </button>
                    </div>

                    <div className="member-days-list">
                      {DIAS_LEITURA.map((diaSemana) => {
                        const dia = sub.dias[diaSemana];

                        if (!dia) {
                          return null;
                        }

                        const membros = Object.values(dia.membros);

                        return (
                          <details
                            className="history-day-block"
                            key={diaSemana}
                            open
                          >
                            <summary>
                              <div>
                                ...
                                <small>
                                  {dia.registros.length} conferência(s)
                                </small>
                              </div>
                            </summary>
                            ...
                            <div className="member-grid">
                              {membros.map((membro) => {
                                const membroStats =
                                  calcularEstatisticas(membro.registros);
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="member-card-metrics">
                                      <em>
                                        {membroStats.total} conf.
                                      </em>

                                      <em>
                                        {membroStats.aprovados} ✅
                                      </em>

                                      <em>
                                        {membroStats.reprovados} ❌
                                      </em>
                                    </div>
                                  </button>
                                );
        ...
        <div className="modal-overlay">
          <div className="modal-card">
            ...
            <p>
              ...
              {" "}
              <strong>
                {modalAprovacao.capitulo?.titulo || "capítulo"}
              </strong>
            </p>
            ...
            <label className="field field-full">
              ...
              <textarea
                ...
                onChange={(event) =>
                  setMotivoAprovacao(event.target.value)
                }
...
function TelaMembro({
  ...
  const painel =
    calcularPainelSemanal(membro.registros);
      ...
      <div className="weekly-panel">
        {painel.map((dia) => (
          <div className="weekly-day-card" key={dia.dia}>
            ...
            <small>
              {dia.aprovados} aprovadas · {dia.reprovados} reprovadas
            </small>
          </div>
        ))}
      </div>
    ...
    <article className={`history-card status-${registro.statusGeral}`}>
      <div className="history-card-header">
        <div>
          ...
          <h4>
            {registro.nome || registro.user || "Sem nome"}
          </h4>

          <p>
            {registro.diaSemana || "Sem dia"}
            {" · "}
            {registro.subNome || "Sem sub"}
          </p>
        </div>

        <strong
          className={`status-pill ${definirClasseStatus(
            registro.statusGeral
          )}`}
        >
      ...
      <div className="history-card-grid">
        ...
        <div>
          ...
          <strong>
            {(registro.resumoStatus?.reprovado || 0) +
              (registro.resumoStatus?.erro || 0)}
          </strong>
        </div>
  ...
  return (
    <div className="history-work-detail">
      <div className="history-work-item">
        <div>
          <strong>
            {leitura.obraEncontrada?.nome || leitura.obraInformada}
          </strong>
    ...
    <div className={`history-chapter-card status-${capitulo.status}`}>
      ...
      <div className="history-chapter-metrics">
        ...
        <div>
          ...
          <strong>
            {capitulo.totalComentarios || 0}/{capitulo.comentariosMinimos || 0}
          </strong>
        </div>
        ...
        <div>
          ...
          <strong>
            {formatarTempoSegundos(capitulo.tempoReal?.totalSegundos || 0)}
          </strong>
        </div>
      </div>
      ...
      {capitulo.comentarios?.length > 0 ? (
        <details className="comments-preview">
          <summary>Ver comentários</summary>

          <div className="comments-list">
            {capitulo.comentarios.map((comentario) => (
              <div className="comment-preview-card" key={comentario.id}>
                <strong>
